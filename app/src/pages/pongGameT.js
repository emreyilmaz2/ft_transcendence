...
export default function ponggameT() {
...
export function runGameT() {
    ...
    let player1 = JSON.parse(localStorage.getItem('player1NameT'));
    let player2 = JSON.parse(localStorage.getItem('player2NameT'));
    let player3 = JSON.parse(localStorage.getItem('player3NameT'));
    let player4 = JSON.parse(localStorage.getItem('player4NameT'));
    let match1Winner = localStorage.getItem('match1Winner');
    let match2Winner = localStorage.getItem('match2Winner');
    let tournamentRound = parseInt(localStorage.getItem('tournamentRound')); // tournamentRound değişkenini burada tanımlayın
    ...
    function predictBallFuturePosition(ball, deltaTime) {
        ...
        for (let t = 0; t < deltaTime; t++) {
            ...
            if (futurePositionY <= 0 || futurePositionY >= planeHeight) {
    ...
    function buildText()
    {
        if (scoreboard && tournamentRound === 1) {
            scoreboard.textContent = `${player1}: ${p1Points} - ${p2Points} :${player2} `;
        }
        else if (scoreboard && tournamentRound === 2) {
            scoreboard.textContent = `${player3}: ${p1Points} - ${p2Points} :${player4} `;
        }
        else {
            scoreboard.textContent = `${match1Winner}: ${p1Points} - ${p2Points} :${match2Winner} `;
        }
    }
    function animate() {
        // Ball movement
...
async function modalcreateLogin(p1Points,p2Points) {
    ...
    let tournamentRound = parseInt(localStorage.getItem('tournamentRound'));
    let winner;
    if (tournamentRound === 1) {
        if (p1Points > p2Points)
            winner=localStorage.getItem('player1NameT');
        else if(p1Points < p2Points)
            winner=localStorage.getItem('player2NameT');
        localStorage.setItem('match1Winner', winner);
    }
    else if (tournamentRound === 2) {
        if (p1Points > p2Points)
            winner=localStorage.getItem('player3NameT');
        else if(p1Points < p2Points)
            winner=localStorage.getItem('player4NameT');
        localStorage.setItem('match2Winner', winner);
    }
    else {
        if (p1Points > p2Points)
            winner=localStorage.getItem('match1Winner');
        else if(p1Points < p2Points)
            winner=localStorage.getItem('match2Winner');
        localStorage.setItem('tournamentWinner', winner);

        localStorage.removeItem('tournamentRound');
        localStorage.removeItem('match1Winner');
        localStorage.removeItem('match2Winner');
    ...
    if (tournamentRound === 1) {
        winnerText = `Birinci Maçın Kazananı: ${winner}`;
    } else if (tournamentRound === 2) {
        winnerText = `İkinci Maçın Kazananı: ${winner}`;
    } else {
        winnerText = `Turnuvanın Kazananı: ${winner}`;
        // Clear local storage after displaying the final winner
        localStorage.removeItem('match1Winner');
        localStorage.removeItem('match2Winner');
        localStorage.removeItem('tournamentWinner');
        localStorage.removeItem('tournamentRound');
    }
    modalBody.innerHTML = winnerText;
    ...
    closeButtonFooter.textContent = 'Next';
